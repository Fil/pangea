...
svg.append("g")
  ...
  .call(g => g.select(".domain").attr("stroke", "black").attr("transform", `translate(${x(0)},0)`));
...
const data = d3.csvParse(await FileAttachment("/data/politifact.csv").text(), ({speaker: name, ruling: category, count: value}) => categories[category] ? {name, category: categories[category], value: +value} : null);
