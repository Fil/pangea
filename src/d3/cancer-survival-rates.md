...
A [slope chart](./slope-chart), with additional intermediate steps. The values here are survival rate (in percents) for various cancers. Based on [Tufte](https://www.edwardtufte.com/bboard/q-and-a-fetch-msg?msg_id=0003nk).
...
svg.append("g")
  ...
  .join("g")
    ...
    .call(g => g.append("text").attr("fill", "currentColor").text((d) => d))
...
```js echo
const data = FileAttachment("/data/cancer.csv").csv({typed: true})
```
