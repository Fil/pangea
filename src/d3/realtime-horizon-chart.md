...
This [horizon chart](./horizon-chart) shows realtime [random walks](https://en.wikipedia.org/wiki/Random_walk). Only the new values are drawn on each tick, while the old values are shifted to the left.
...
const chart = display(Object.assign(div.node(), {
...
```js echo
import {when} from "/components/Promises.js"
```
