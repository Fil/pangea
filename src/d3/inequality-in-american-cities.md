---
source: https://observablehq.com/@d3/inequality-in-american-cities
index: true
---
...
```js
{
  ...
  const legend = htl.svg`<svg width="180" height="33" style="display: block; font: 10px sans-serif;">
  <defs>
  ...
  <text x="4" y="16.5" dy="0.36em" text-anchor="start" fill="currentColor">1980</text>
  <text x="176" y="16.5" dy="0.36em" text-anchor="end" fill="currentColor">2015</text>
</svg>`;
  display(legend);
}
```
...
```js echo
const data = FileAttachment("/data/metros.csv").csv({typed: true});
```
