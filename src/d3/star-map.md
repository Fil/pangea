...
const svg = d3
  ...
  .attr(
    "style",
    `display: block; margin: 0 -14px; width: ${width}px; max-width: 100vw; height: auto; font: 10px sans-serif; color: white; background: radial-gradient(#081f4b 0%, #061616 100%);`
  )
...
svg
  ...
  .join("line")
  .datum((d) => [projection([d / 4, 0]), projection([d / 4, d % 60 ? -1 : -2])])
  .attr("x1", ([[x1]]) => x1)
  .attr("x2", ([, [x2]]) => x2)
  .attr("y1", ([[, y1]]) => y1)
  .attr("y2", ([, [, y2]]) => y2);
...
```js echo
const data = d3
  .csvParse(await FileAttachment("../data/stars.csv").text(), (d) => {
    d3.autoType(d);
...
${nominative[constellation]}`
      : `
${greek_letter.replace(/[a-z]+/g, (w) => letters[w]).replace(/\d/g, (c) => "⁰¹²³⁴⁵⁶⁷⁸⁹"[c])} ${genitive[constellation]}`
  }`;
}
```
