...
form.o.value = formatLocation(form.value);
const location = view(form);
```

```js
const date = view(Scrubber(d3.utcDays(new Date(Date.UTC(2019, 0, 1)), new Date(Date.UTC(2020, 0, 1))), {
...
```js
import Solar from "npm:solar-calculator@0.2";
```

```js
const solar = Solar(location);
```
